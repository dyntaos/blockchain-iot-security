...
class blockchain-sec::Blockchain-Sec-Lib {
	...
	public:
		Blockchain-Sec-Lib(std::string const ipc_path, std::string const eth_sec_contract_addr) {
		...
		std::string eth_call(std::string abi_data) {
			std::string json_request = "{""jsonrpc"":""2.0"",""method"":""eth_call"",""params"":[{""to"":""" + this->eth_sec_contract_addr + """,""gasPrice"":0,""data"":""" + abi_data + """}],""id"":1}";
			cout << "eth_call()\n";
